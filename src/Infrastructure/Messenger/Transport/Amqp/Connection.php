...
class Connection
{
     ...
     */
    public function publish(string $body, ?string $routingKey, int $flags = AMQP_NOPARAM, array $options = []): void
    {
        ...
        if (null === $routingKey) {
            $routingKey = $this->queueConfiguration['routing_key'] ?? null;
        }

        $this->exchange()->publish($body, $routingKey, $flags, $options);
    }
    ...
    private function shouldSetup(): bool
    {
        return !\array_key_exists(
            ...
            $this->connectionCredentials
                ...
                $this->connectionCredentials['auto-setup'],
                [false, 'false'],
                true
            );
    }
    ...
    public function setup(): void
    {
        ...
        if (\is_string($this->exchange()->getName()) && '' !== $this->exchange()->getName()) {
            $this->exchange()->declareExchange();
        }

        if ('amq.rabbitmq.reply-to' !== $this->queue()->getName()) {
    ...
    public function channel(): \AMQPChannel
    {
        if (null === $this->amqpChannel) {
            ...
            $connectMethod = 'true' === ($this->connectionCredentials['persistent'] ?? 'false') ? 'pconnect'
                : 'connect';
